...
import Validation from '../signup-in/SignupValidation';
...
function Signup() {
    ...
    const handleSubmit = (event) => {
        ...
        if (err.name === "" && err.email === "" && err.password === "") {
            axios.post('http://localhost:700/signup', values)
    ...
    const handleLogin = (googleData) => {
        ...
        let img = googleData.profileObj.imageUrl + '';
        let email = googleData.profileObj.email + ''


        axios.post('http://localhost:700/googlesignup', { name, email, img })
            .then(res => {
